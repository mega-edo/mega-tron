...
def render_inline_self_eval_contract() -> str:
    ...
    return (
        "### Self-evaluation\n"
        "For each skill listed above that influenced your reply, append "
        "one tag at the end of your message. Before writing each verdict, "
        "answer two questions about the skill (think silently, do not "
        "narrate):\n"
...
def render_install_tagging_guide(*, hook_name: str) -> str:
    ...
    """
    return (
        "Tagging contract: for each skill in the catalog that influenced "
        "your reply, append one tag at the end of your final message. "
        "Before assigning a verdict, answer two questions about the "
        "skill (silently):\n"
        "  (1) Did your final answer use content from this skill?\n"
        "  (2) Is the approach this skill RECOMMENDS correct for the "
        "user's task?\n"
